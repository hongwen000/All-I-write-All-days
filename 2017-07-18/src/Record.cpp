...
bool Record::startRecord(std::string tmp_cmd,
    std::mutex& mut,
    std::string& cmd)
{
    mut.unlock();
    ...
    if (!isStoped() || _video.isOpened()) {
        std::cerr << "Previous Recording is forced to stop to start the new one"
                  << std::endl;
        _video.release();
        _flag_stop = true;
    }
    _video = cv::VideoWriter(file_name, _video_format, _video_fps,
        cv::Size(_frame_width, _frame_height), _is_colored);
    if (!_video.isOpened()) {
...
bool Record::close()
{
    if (_video.isOpened() || !isStoped())
        throw std::logic_error(
            "invariant broken: _video is open or still recording");
    _camera.release();
    return !_camera.isOpened();
}
