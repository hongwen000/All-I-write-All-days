...
bool reflexive(const Relation& R, const Set& A)
{
    ...
    return isSelf(A.size(), n);
                ...
                bool transitive(const Relation &R, const Set &A)
                {
                  for (auto ita = A.begin(); ita != A.end(); ++ita) {
                    for (auto itb = A.begin(); itb != A.end(); ++itb) {
                      for (auto itc = A.begin(); itc != A.end(); ++itc) {
                        if (R.find(make_pair(*ita, *itb)) != R.end() &&
                            R.find(make_pair(*itb, *itc)) != R.end())
                          if (R.find(make_pair(*ita, *itc)) == R.end())
                            return false;
                      }
                    }
                  }
                ...
                int main() {
                    cout << "请输入定义域元素个数" << endl;
                    int N;
                    cin >> N;
                    Set a;
                    int ele;
                    cout << "请输入定义域中所有元素" << endl;
                    while(N--) {
                        cin >> ele;
                        a.insert(ele);
                    }
